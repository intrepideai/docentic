---
name: docentic
description: Make a repo agent-friendly. Use when the user says "docentic this repo", "scaffold docentic", "make this repo agent-friendly", "give this repo AGENTS.md", or similar — anything about adding standardized agent-readable documentation to a codebase. Scaffolds AGENTS.md + a docs/ spine + research pipeline + maintain-repo skill.
---

# docentic

> Your agent guide through any codebase. <https://github.com/intrepideai/docentic>

When invoked, you scaffold the [docentic](https://github.com/intrepideai/docentic) template into a repo and optionally fill in the content TODOs.
...
- "Add the docentic template here"
...
Default to the current working directory. If unclear, ask the user briefly: "Scaffold docentic into `<cwd>`?" — wait for confirmation if the directory looks suspicious (e.g. they're in their home directory rather than a repo).

### Step 2 — Run the scaffold

```bash
cd <target-repo>
npx -y @intrepideai/docentic init
```

This commits a `docentic/template-scaffold` branch and (if `gh` CLI is configured) opens a PR. ~50 files are created: `AGENTS.md` at root, the `docs/` spine, `.agents/`, `.claude/skills/maintain-repo/`, `scripts/llm-docs/`, `research/`.
...
If yes:
1. Read [`prompts/bootstrap.md`](https://github.com/intrepideai/docentic/blob/main/prompts/bootstrap.md) from the docentic repo
...
3. Commit on a `docentic/populate-content` branch and open a PR
...
If yes:
1. Read [`prompts/config-seeder.md`](https://github.com/intrepideai/docentic/blob/main/prompts/config-seeder.md)
...
Don't keep going. The maintenance loop is a separate concern handled by the `maintain-repo` skill that `docentic init` scaffolds into the repo.
...
- Bootstrap prompt: <https://github.com/intrepideai/docentic/blob/main/prompts/bootstrap.md>
- Config seeder: <https://github.com/intrepideai/docentic/blob/main/prompts/config-seeder.md>
- Validate a scaffolded repo: `npx -y @intrepideai/docentic check`
