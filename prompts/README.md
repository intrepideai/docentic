...
LLM prompts that pair with the `docent` CLI. Copy-paste into any agent (Claude, ChatGPT, Cursor, Codex, Gemini, etc.) when you want to drive a step manually instead of via the CLI.

## Three orchestrator-facing prompts at this level

| Prompt | When to use | Recommended model |
|---|---|---|
| [`bootstrap.md`](./bootstrap.md) | Once per repo, right after `docent init` — fills in AGENTS.md + docs/* TODOs from reading the codebase | Opus-class |
...
| [`conflict-resolver.md`](./conflict-resolver.md) | When `docent check` reports a hash conflict on a generated file | Opus-class |

## Per-task prompts (scaffolded into each repo)

When you run `docent init`, the per-task prompts get scaffolded INTO your repo at `scripts/llm-docs/prompts/`:
...
1. Run `docent init <repo>` (or whatever step prompted you to invoke this prompt)
...
`docent populate` reads `ANTHROPIC_API_KEY` from `.env` and runs the Bootstrap prompt automatically. See the top-level [README](../README.md) for setup.
